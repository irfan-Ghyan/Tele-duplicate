...
//   //   { title: "duration", description: "120 Min" }, 
...
const Page = ({ params } ) => {
  ...
  const [bookingDetails, setBookingDetails] = useState([
    ...
    { key: "duration", title: "Duration", description: "120 Min" },
  ]);
  ...
  const [generalError, setGeneralError] = useState("");
  const [bookingErrors, setBookingErrors] = useState([]);
  const [availableSIMs, setAvailableSIMs] = useState(null); // To store available SIMs for the clicked slot
  ...
  const fetchEventDetails = async () => {
    try {
      ...
      const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
      const url = `${baseUrl}/api/content/sections/${id}`;
      let response = await doGetCall(url);
      const data = await response.json();
      console.log("Fetched Event Details:", data);
      setEventDetails(data);
  ...
  const handleButtonClick = (timeKey, timeValue, sims) => {
    setActiveTime(timeKey);
    updateBookingDetail("time", timeValue);
    setAvailableSIMs(sims); // Save the available SIMs for the selected time slot
