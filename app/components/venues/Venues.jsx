...
//         if (!response.ok) {
//           throw new Error(t('Dome.ErrorFetchingData'));
//         }

//         const data = await response.json();
//         const domeSection = data?.data?.sections.find((section) => section.title === "Venues Branding");
...
//           const imagesResponse = await getImageCall(`${baseUrl}/api/content/getImages/Venues Branding`);
//           const imagesData = await imagesResponse.json();

//           if (imagesData.success) {
//             const images = imagesData.data.map((image) => image.url);
//             const updatedDomes = images.map((url, index) => ({
//               ...slidesArray[index % slidesArray.length],
//               imageUrl: url,
//             }));
...
//         }
//       } catch (err) {
//         setError(err.message || t('Dome.ErrorFetchingData'));
//       } finally {
//         setLoading(false);
//       }
//     };
...
//         {loading && <p className="text-[#e3ce90] text-xl">{t('Dome.Loading')}</p>}
...
//               >
//                 <span className="button-slanted-content">{t('Dome.DiscoverTheDome')}</span>
//               </Link>
//             </div>
//           </div>
//         ))}
