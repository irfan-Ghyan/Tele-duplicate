...
//         const domeSection = data?.data?.sections.find((section) => section.title === 'Conference');
//         if (domeSection) {
...
//   return (
//     <div className={`w-full py-[40px] md:py-[50px] lg:py-[100px] bg-white border-t-2 border-color-200  ${language === 'ar' ? 'text-right' : 'text-left'}`}>
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const domeSection = data?.data?.sections.find(
          (section) => section.title === 'Conference'
        );
        if (domeSection) {
          const groupedSlides = domeSection.section_fields.reduce((acc, field) => {
            const match = field.key.match(/(title|description)(\d+)/);
