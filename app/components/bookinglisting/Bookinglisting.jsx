...
const BookingListing = () => {
  ...
  const generateTimeSlots = () => {
    const duration = 20; // Fixed interval of 20 minutes for all booking types
    ...
    // If the selected date is today, start from the nearest future rounded 20-minute slot
    if (selectedDate.toDateString() === now.toDateString()) {
      const currentMinutes = now.getHours() * 60 + now.getMinutes();
      ...
      // Start from 9:00 AM for future dates
      startTime = new Date(selectedDate.setHours(9, 0, 0));
    }
    ...
    // Generate time slots with a 20-minute interval
    while (startTime < endTime) {
      slots.push(startTime.toTimeString().slice(0, 5)); // Format HH:MM
      startTime = new Date(startTime.getTime() + duration * 60 * 1000);
    }
